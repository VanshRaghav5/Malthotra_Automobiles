...
async function seed() {
  ...
  const prodRes = await supabase.from('products').insert(productInserts);
  console.log('Products inserted:', prodRes.error ? 'error: ' + prodRes.error.message : `${productInserts.length} ok`);

  // Create slots for Oil Change service
  ...
  if (oilServiceId) {
    ...
    console.log('Slots:', slotRes.error ? 'error: ' + slotRes.error.message : `${slots.length} inserted`);
  }

  // Verify counts
  const { data: cats } = await supabase.from('categories').select('count');
  console.log('\nFinal counts - Categories:', cats?.[0]?.count);
  const { data: prods } = await supabase.from('products').select('count');
  console.log('Products:', prods?.[0]?.count);
  const { data: svcs } = await supabase.from('services').select('count');
  console.log('Services:', svcs?.[0]?.count);
  const { data: slots } = await supabase.from('availability_slots').select('count');
